...
def get_targets_af(cfg, gt_bboxes, gt_labels, action_nums):
    ...
    expanded_regress_ranges = [
        points[i].new_tensor(regress_ranges[i])[None].repeat(  # [None] is used to add "[]" outsides
            points[i].size(0), 1) for i in range(num_levels)
    ]
...
def get_target_single(gt_bboxes, gt_labels, action_nums, points, regress_ranges,
    ...
    if cfg.TRAIN.CENTER_SAMPLING:
        ...
        for lvl_idx, num_points_lvl in enumerate(num_points_per_lvl):
            ...
            stride[lvl_begin:lvl_end] = cfg.MODEL.TEMPORAL_STRIDE[lvl_idx]
            lvl_begin = lvl_end

        center_mins = centers - stride * cfg.TRAIN.CENTER_SAMPLE_ALPHA
        center_maxs = centers + stride * cfg.TRAIN.CENTER_SAMPLE_ALPHA
    ...
    inside_regress_range = (
             ...
             & (max_regress_distance <= regress_ranges[..., 1])) # 各层的回归范围要在各层之内，其实也就是将gt分散到各层
